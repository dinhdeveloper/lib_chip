...
class PersonDetails : Parcelable {
    ...
    var optionalData1: String? =
        null /* NOTE: holds personal number for some issuing states (e.g. NL), but is used to hold (part of) document number for others. */
    ...
    constructor(`in`: Parcel) {
        ...
        this.gender =
            if (`in`.readInt() == 1) Gender.valueOf(`in`.readString()!!) else Gender.UNKNOWN
    }
