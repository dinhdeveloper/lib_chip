...
import unit.chip.lib_unit_chip.model.EACCredentials
...
object CardNfcUtils {
    ...
    private fun toBitmap(imageLength: Int, inputStream: InputStream, mimeType: String): Bitmap {
        ...
        return decodeImage(byteArrayInputStream, imageLength, mimeType)
    }
    ...
    private fun decodeImage(
        ...
        // Đọc dữ liệu từ ByteArrayInputStream và tạo Bitmap
        val buffer = ByteArray(imageLength)
        inputStream.read(buffer, 0, imageLength)

        // Giải mã hình ảnh sử dụng BitmapFactory
        val options = BitmapFactory.Options()
        options.inPreferredConfig = Bitmap.Config.ARGB_8888 // Cấu hình màu ảnh (có thể điều chỉnh)
        options.inSampleSize = 1 // Có thể điều chỉnh để giảm kích thước ảnh

        // Giải mã và trả về Bitmap
        return BitmapFactory.decodeByteArray(buffer, 0, imageLength, options)
    }
    ...
    fun getCertificateChain(
        ...
        try {
            ...
            val builder = CertPathBuilder.getInstance("PKIX", "BC")//Spungy castle
            ...
            try {
                ...
                Log.w(TAG, "PKIX ${cpbe.printStackTrace()}")
            }
