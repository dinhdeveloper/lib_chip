...
import unit.chip.lib_unit_chip.common.CardFileInputStr
import unit.chip.lib_unit_chip.common.StringUtils
import unit.chip.lib_unit_chip.model.FeatureEiDStatus
import unit.chip.lib_unit_chip.model.VerificationEiDStatus
...
class CardEidNFC  @Throws(GeneralSecurityException::class) private constructor(){
    private var digest: MessageDigest? = null
    private var service: PassportService? = null
    val features: FeatureEiDStatus = FeatureEiDStatus()
    val verificationStatus: VerificationEiDStatus = VerificationEiDStatus()
    ...
    constructor(ps: PassportService?, mrzInfo: MRZInfo, maxBlockSize: Int) : this(){
        ...
        try {
            try {
                ...
                for (securityInfo in securityInfos) {
                    if (securityInfo is PACEInfo) {
                        features.hasSAC = (FeatureEiDStatus.Verdict.PRESENT)
                    }
                }
            } catch (e: Exception) {
            ...
            hasSAC = features.hasSAC == FeatureEiDStatus.Verdict.PRESENT
            if (hasSAC) {
                try {
...
//            dataAdditionalPersonDetails = ConvertByteArrayToData(data).convert()
            ...
            try {
                ...
                if (isSACSucceeded) {
                    verificationStatus.secureAccessControl = VerificationEiDStatus.Verdict.SUCCEEDED
                    features.hasBAC = FeatureEiDStatus.Verdict.UNKNOWN
                    verificationStatus.bac = VerificationEiDStatus.Verdict.NOT_CHECKED
                } else {
                    /* We failed SAC, and we failed BAC. */
                    features.hasBAC = FeatureEiDStatus.Verdict.NOT_PRESENT
                    verificationStatus.bac = VerificationEiDStatus.Verdict.NOT_PRESENT
                }
            } catch (e: Exception) {
                Log.i(TAG, "Attempt to read EF.COM before BAC failed with: " + e.message)
                features.hasBAC = FeatureEiDStatus.Verdict.PRESENT
                verificationStatus.bac = VerificationEiDStatus.Verdict.NOT_CHECKED
            }

            /* If we have to do BAC, try to do BAC. */
            val hasBAC = features.hasBAC == FeatureEiDStatus.Verdict.PRESENT
            ...
            if (hasBAC && !(hasSAC && isSACSucceeded)) {
                ...
                try {
                    ...
                    verificationStatus.bac = VerificationEiDStatus.Verdict.SUCCEEDED
                } catch (e: Exception) {
                    verificationStatus.bac = VerificationEiDStatus.Verdict.FAILED
                }
            ...
            var hashResults: MutableMap<Int, VerificationEiDStatus.HashMatchResult>? =
                verificationStatus.hashResults as MutableMap<Int, VerificationEiDStatus.HashMatchResult>?
            if (hashResults == null) {
                hashResults = TreeMap<Int, VerificationEiDStatus.HashMatchResult>()
            }
            ...
            if (sodFile != null) {
                ...
                for (dgNumber in dgNumbers) {
                    ...
                    var hashResult: VerificationEiDStatus.HashMatchResult? = hashResults[dgNumber]
                    if (hashResult != null) {
                        continue
                    }
                    if (dgNumbersAlreadyRead.contains(dgNumber)) {
                        ...
                        hashResult = VerificationEiDStatus.HashMatchResult(storedHash!!, null)
                    }
                    hashResults[dgNumber] = hashResult!!
                }
            }
            verificationStatus.passiveAuthentication = (VerificationEiDStatus.Verdict.UNKNOWN)

            /* Check EAC support by DG14 presence. */
            if (dgNumbers.contains(14)) {
                features.hasEAC = FeatureEiDStatus.Verdict.PRESENT
                if(isChipAuthenticationInfoAvailable(ps, mrzInfo, dg14File, sodFile)) {
                    features.hasCA = FeatureEiDStatus.Verdict.PRESENT
                }else{
                    features.hasCA = FeatureEiDStatus.Verdict.NOT_PRESENT
                }
            } else {
                features.hasEAC = FeatureEiDStatus.Verdict.NOT_PRESENT
                features.hasCA = FeatureEiDStatus.Verdict.NOT_PRESENT
            }

            val hasCA = features.hasCA == FeatureEiDStatus.Verdict.PRESENT
            if (hasCA) {
                try {
                    val eaccaResults = doEACCA(ps, mrzInfo, dg14File, sodFile)
                    verificationStatus.chipAuth = (VerificationEiDStatus.Verdict.SUCCEEDED)
                } catch (e: Exception) {
                    verificationStatus.chipAuth = (VerificationEiDStatus.Verdict.FAILED)
                }
            ...
            if (dgNumbers.contains(15)) {
                features.hasAA = FeatureEiDStatus.Verdict.PRESENT
            } else {
                features.hasAA = FeatureEiDStatus.Verdict.NOT_PRESENT
            }
            val hasAA = features.hasAA == FeatureEiDStatus.Verdict.PRESENT
            if (hasAA) {
                try {
                    ...
                    verificationStatus.activeAuth = (VerificationEiDStatus.Verdict.NOT_CHECKED)
                }

            } else {
                /* Feature status says: no AA, so verification status should say: no AA. */
                verificationStatus.activeAuth = (VerificationEiDStatus.Verdict.NOT_PRESENT)
            }
    ...
    private fun verifyHash(dgNumber: Int): VerificationEiDStatus.HashMatchResult? {
        var hashResults: MutableMap<Int, VerificationEiDStatus.HashMatchResult>? =
            verificationStatus.hashResults as MutableMap<Int, VerificationEiDStatus.HashMatchResult>?
        if (hashResults == null) {
            hashResults = TreeMap<Int, VerificationEiDStatus.HashMatchResult>()
    ...
    private fun verifyHash(dgNumber: Int, hashResults: MutableMap<Int, VerificationEiDStatus.HashMatchResult>): VerificationEiDStatus.HashMatchResult? {
        ...
        try {
            ...
            verificationStatus.passiveAuthentication = (VerificationEiDStatus.Verdict.FAILED)
            return null
        }
        ...
        try {
            ...
            if (abstractTaggedLDSFile == null && verificationStatus.eac != VerificationEiDStatus.Verdict.SUCCEEDED
                ...
                val hashResult = VerificationEiDStatus.HashMatchResult(storedHash!!, null)
                hashResults[dgNumber] = hashResult
                return hashResult
            }
            if (abstractTaggedLDSFile == null) {
        ...
        try {
            ...
            val hashResult = VerificationEiDStatus.HashMatchResult(storedHash!!, computedHash)
            hashResults[dgNumber] = hashResult
    ...
    // VERIFY
    ///////////////////////////////////////////////////////////////////////////
    fun verifySecurity(): VerificationEiDStatus {
    ...
    private fun verifyDocumentSigning() {
        try {
            verificationStatus.documentSigning = (VerificationEiDStatus.Verdict.UNKNOWN)
            ...
            if (checkDocSignature(docSigningCert)) {
                verificationStatus.documentSigning = (VerificationEiDStatus.Verdict.SUCCEEDED)
            } else {
                verificationStatus.documentSigning = (VerificationEiDStatus.Verdict.FAILED)
            }
        } catch (nsae: NoSuchAlgorithmException) {
            verificationStatus.documentSigning = (VerificationEiDStatus.Verdict.FAILED)
    ...
    private fun verifyCountrySigning() {
        try {
            ...
            if (sodFile == null) {
                verificationStatus.countrysign = (VerificationEiDStatus.Verdict.FAILED)
                return
            }
            ...
            if (docSigningCertificate != null) {
                ...
                verificationStatus.countrysign = VerificationEiDStatus.Verdict.SUCCEEDED
            } else {
            ...
            if (chainDepth > 1 && verificationStatus.countrysign == VerificationEiDStatus.Verdict.UNKNOWN) {
                verificationStatus.countrysign = VerificationEiDStatus.Verdict.SUCCEEDED
            }
    ...
    private fun verifyPassiveAuthentication() {
        ...
        if (verificationStatus.passiveAuthentication == VerificationEiDStatus.Verdict.UNKNOWN) {
            verificationStatus.passiveAuthentication = VerificationEiDStatus.Verdict.SUCCEEDED
        } else {
            /* Update storedHashes and computedHashes. */
            verificationStatus.passiveAuthentication = verificationStatus.passiveAuthentication
        }
    }

    private fun verifyActiveAuth() {
        if (dg15File == null || service == null) {
            verificationStatus.activeAuth = (VerificationEiDStatus.Verdict.FAILED)
            return
        }
        ...
        try {
            ...
            if (verifyActiveAuth(pubKey, digestAlgorithm, signatureAlgorithm, challenge, aaResult.response)) {
                verificationStatus.activeAuth = VerificationEiDStatus.Verdict.SUCCEEDED
            } else {
                verificationStatus.activeAuth = VerificationEiDStatus.Verdict.FAILED
            }
        } catch (cse: CardServiceException) {
